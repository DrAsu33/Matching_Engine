...
* **订单吞吐量**：历史本机测试在开启异步日志回调时约为 **2300 万 orders/s**，更新代码或环境后应重新测量。
* **处理延迟**：Benchmark 独立报告 p50、p90、p99、p99.9 和最大单笔观测延迟。
...
Benchmark 将吞吐量与延迟分成两个独立阶段，并分别创建引擎，避免复用已改变的订单簿状态：

* **批量吞吐量**：通过批量 FFI 路径报告 orders/s 和每单摊销时间；摊销时间不是延迟百分位。
* **采样延迟**：关闭日志，预热后逐笔调用 Rust → C ABI → C++ 路径，默认每 100 单采样一次。
* **延迟指标**：报告 mean、min、p50、p90、p99、p99.9、max，以及 `Instant` 计时器的 p50 基线开销。
* **统计口径**：延迟样本是原始观测值，包含单次 FFI 和计时器开销；样本排序和百分位计算位于计时区间外。
...
* `LATENCY_WARMUP_ORDERS`: 延迟测试开始采样前处理的预热订单数。
* `LATENCY_SAMPLE_EVERY`: 延迟采样间隔；默认每 100 单记录一条样本。
